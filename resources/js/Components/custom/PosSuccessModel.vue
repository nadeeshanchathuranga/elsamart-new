    ...
    .map((product) => {
      return `
        ...
        <tr style="border-bottom: 1px dashed #999;">
          ...
          <td>
  ${product?.marked_price == null || !Number.isFinite(+product.marked_price)
      ? 'N/A'
...
<style>
  ...
  .header img{
    width:300px; height:120px; object-fit:contain;
...
<div class="info-row"> <div> <p>Date & Time:</p> <small>${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}</small> </div> <div> <p>Order No:</p> <small>${props.orderid}</small> </div> </div> <div class="info-row"> <div> <p>Customer:</p> <small>${props.customer?.name || ""}</small> </div> <div> <p>Cashier:</p> <small>${props.cashier?.name || ""}</small> </div> </div>
      ...
      <table>
        <thead>
          <tr style="border-bottom:1px solid black;">
            ...
            <th style="text-align:center; padding:4px;">Marked Price</th>
            <th style="text-align:center; padding:4px;">Our Price</th>
    ...
    <div class="totals">
      ${Number(props.subTotal || 0)
        ? `<div><span>Gross Total</span><span>${(Number(props.subTotal || 0)).toFixed(2)} LKR</span></div>`
        : ""
      }
      ...
      ${Number(props.total || 0)
        ? `<div><span>Net Total</span><span>${(Number(props.total || 0)).toFixed(2)} LKR</span></div>`
        : ""
      }
      ${Number(props.cash || 0)
    ...
    <div class="footer">
      <p>Seasonal Greetings  Special Offer</p>
