   ...
   public function update(Request $request, Product $product)
{
    ...
    try {
        $validated = $request->validate([
            'category_id' => 'nullable|exists:categories,id',
            'supplier_id' => 'nullable|exists:suppliers,id', // Added supplier_id
            'name' => 'required|string|max:255',
            'size_id' => 'nullable|exists:sizes,id',
            'color_id' => 'nullable|exists:colors,id',
            ...
            'barcode' => 'nullable|string|max:100', // Added barcode
        ...
        if ($request->hasFile('certificate')) {
            ...
            $certificateName = 'certificate_' . now()->format('YmdHis') . '.' . $request->file('certificate')->getClientOriginalExtension();
            $certificatePath = $request->file('certificate')->storeAs('certificates', $certificateName, 'public');
            $validated['certificate_path'] = 'storage/' . $certificatePath;
        }
        ...
        return back()->withErrors(['error' => 'Product update failed. Please try again.'])->withInput();
    }
}
    ...
    public function uploadCsv(Request $request)
    {
        ...
        foreach ($data as $row) {
            ...
            Product::create([
                ...
                'marked_price' => is_numeric($row[2]) ? $row[2] : 0,
